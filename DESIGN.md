...
Canvas 与 AI 图片模式共享语音入口和反馈栏，但执行链完全分离。Canvas 指令进入规则解析器与矢量执行器；AI 模式的普通语音只追加提示词，只有用户点击“确认并生成”按钮才发起图片请求，避免语音误识别造成重复请求。两种模式分别维护历史，切换模式不会破坏另一模式作品。
...
- 已通过安全代理接入硅基流动官方生图接口和 Kolors 模型，但暂未本地部署 Diffusion / ControlNet，也未实现真实局部重绘。
