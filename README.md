...
VoiceCanvas AI 是一个纯前端、纯语音控制的中文矢量绘图工具。用户点击一次按钮开启麦克风后，即可通过中文语音指令在 Canvas 上绘制、编辑和导出作品，绘图过程中不需要鼠标或键盘。
...
- **零后端依赖**：使用 HTML、CSS、原生 JavaScript、Canvas API 与 Web Speech API。
...
# 方式二：项目脚本（内部仍使用 Python）
npm run serve

# 方式三：VS Code Live Server
# 在项目根目录启动 Live Server
```

然后访问 `http://localhost:8000`，允许浏览器使用麦克风。
...
**点击开始后没有反应？**  
确认使用 Chrome 或 Edge，并允许当前站点访问麦克风。语音识别服务还可能受网络环境影响。
